...
void hiloEnvia(int numSolicitud, char *IP){
	int archivo2;
	int desicion;	
	unsigned int id = 0;
...
void hiloEnvia2(int numSolicitud2, char *IP2){
	int archivo3;
	int desicion2;	
	unsigned int id2 = 0;

	archivo3 = open("registros.txt", O_RDWR, 0666);
	...
	long int resp2;
	struct registro regN2;
	for(int x = 0; x < numSolicitud2; x++){		
		read(archivo3, &regN2, sizeof(regN2));			
		desicion2 = atoi(&regN2.celular[9]);		
		printf("Digito: %d\n", desicion2);
		printf("Celular: %s\n", regN2.celular);

		if(desicion2 % 2 != 0){
			printf("Enviando impar...\n");
			memcpy(&resp2,solicitud2.doOperation(IP2,7200,3,(char*)&regN2,id2),sizeof(int));
			id2++;
		}
		
		printf("Paquete hilo 2: %d \n", id2);		
	}
	close(archivo3);
	solicitud2.getSocket()->~SocketDatagrama();
}
...
void principalT(char *direccion, char *direccion2, char *votos){
 ...
 //    for(int i = 0; i < 2; i++){
	...
	for(int i = 0; i < 2; i++){
		if(i == 0){
			server[i] = thread(hiloEnvia, numVotos, direccion);
		}else{
			server[i] = thread(hiloEnvia2, numVotos, direccion2);
		}
